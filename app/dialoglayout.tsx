            ...
            return <>
                <div className={"absolute left-1/2 top-1/2 -ml-28 -mt-32"}>
                    <h1 className={"hover:cursor-pointer"}
                        onClick={() => router.push(dialogData.story[0].action.route)}>{dialogData.story[0].action.text}</h1>
                </div>
            </>
        }
        return null
        ...
        if (dialogData.story[0].action === undefined) {
            return <div className={"h-full"}>
                    ...
                    <div className="w-3/5 flex flex-col items-center mb-5">
                        ...
                        {dialog[currentDialogIndex].image && (
                            <div className="flex justify-center mb-4 w-full">
                                <Image src={dialog[currentDialogIndex].image as string} alt="Dialog Bild" width={300}
                                       height={100}/>
                            </div>
                        )}
                  ...
                  text-center`}
                                    ...
                                    >
                                        {feedback.some(fb => fb.index === index) && (
                                            <span
                                                className={`mr-2 ${feedback.find(fb => fb.index === index)?.isCorrect ? 'text-white' : 'text-white'}`}>
                ...
                <div className="absolute bottom-0 right-0 text-right pb-6 pr-6 z-10">
                    {actionButton && !dialog[currentDialogIndex].question && showActionButton && React.cloneElement(actionButton, {onClick: handleNext})}
                </div>
            </div>
        }
        return null
